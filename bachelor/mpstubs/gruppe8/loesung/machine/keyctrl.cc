...
/*   set_led (led::caps_lock, false);
   ...
   set_led (led::num_lock, false);    					ABER WARUM???   */				
...
/* Hier muesst ihr selbst Code vervollstaendigen */ 
	...
	unsigned char stat = ctrl_port.inb();
	...
	if (stat & outb) {
		...
		if (!(stat & auxb)) {										//auxb hat nur an einer Stelle eine 1, d.h., wenn nicht Maus, ist Verundung ueberall 0
			if (key_decoded()) {
				return gather;
			}
		}
...
void Keyboard_Controller::set_repeat_rate (int speed, int delay) {
	int stat = ctrl_port.inb();

	do {
		stat = ctrl_port.inb();
	} while ((stat & inpb) != 0);

	data_port.outb(kbd_cmd::set_speed);		//Befehl in Dataport schreiben
	
	do {
		stat = data_port.inb();
	} while (stat != kbd_reply::ack);		// Ack abwarten

	char cmd = delay;				// Parameter schicken
	cmd = cmd << 5;
	cmd = cmd | speed;
...
// 
	...
	if (!on) {
		leds &= ~led;						// die leds, die in led angegeben sind, ausschalten
	}
	else {
		leds |= led;
	}
	...
	data_port.outb(kbd_cmd::set_led);		// Befehl schicken
