...
/* zaehlt Anzahl der Strings, die in den Speicher geladen werden,
 * wird spaeter fuer printArr benoetigt, deshalb eine static-Variable
 * Koennte z.B. vermieden werden, indem man alles in die main()-Funktion
 * schreibt. */
...
char** readInput()
{
	...
	while ((tmp = fgetc(stdin)) != EOF)
	{
		if (tmp == '\n')
		{
			if (charCount != 0)	/* leere Zeile ignorieren */
			{
				inputArr[strCount][charCount] = '\0';
				/* ueberflussigen alloziierten Speicher frei machen: */ 
		...
		if (charCount > 100)
		{
			perror("Wort mit mehr als 100 Zeichen ignoriert!");
			charCount = 0;
...
{	
	...
	/* Speicher vor Beenden des Programms wieder freigeben */
